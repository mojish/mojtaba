...
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => $pmembersiddd[1] ,"parse_mode" =>"HTML"));	
    ...
    apiRequest("sendphoto", array('chat_id' => $membersidd[$y], "photo" => $substtr,"caption" =>$caption));
		...
		$memcout = count($membersidd)-1;
	 	apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "📦 پیام شما به  ".$memcout." مخاطب ازسال شد.","parse_mode" =>"HTML",'reply_markup' => array(
...
if($chat_id==$admin){
  apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => ' سلام قربان حوش آمدید😉.',"parse_mode"=>"MARKDOWN", 'reply_markup' => array(
        'keyboard' => array(array('🗣 Send To All'),array('⚓️ Help','👥 Members','❌ Blocked Users'),array("Settings ⚙")),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true)));
    ...
    file_put_contents('msgs.txt',$starttext." ".$pmembersiddd[1]);
apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" =>"📝پیام خوش آمد گویی به 👇".$starttext.""."👆تغییر یافت."));
    ...
    file_put_contents('msgs.txt',$pmembersiddd[0]."-!-@-#-$"."".$starttext);
apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" =>"📝پیام پیش فرض شما به 👇".$starttext.""."👆تغییر یافت."));
...
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => $pmembersiddd[1] ,"parse_mode" =>"HTML"));	
    		 ...
    		 apiRequestJson("sendMessage", array('chat_id' => $chat_id,"parse_mode"=>"HTML", "text" => 'یکی از گزینه ها را انتخاب کنید—---------------------------------------------🔶🔸 Clean Members🔶🔸پاک کردن لیست مخاطبین🔷🔹Clean Block List🔷🔹پاک کردن لیست سیاهدر صورت انصراف Back را بزنید.', 'reply_markup' => array(
        ...
        'resize_keyboard' => true)));
    		...
    		apiRequest("sendMessage", array('chat_id' => $admin, "text" => "`برای پاسخ پیام مورد نظر را ریپلای کنید`🔷 لیست کامنت های موجود :🔹`1.` */ban* قرار دادن مخاطب در لیست سیاه(با ریپلای) —------------------------------🔹`2. `*/unban * پاک کردن مخاطب از لیست سیاه(با ریپلای)—------------------------------🔹`3. `*/setstart *اگر شخصی وارد ربات شما شود و */start* بزند این پیام  برای او ارسال میشود           */setstart* بزنید و در ادامه ی آن متن مورد نظر خود را بنویسید.مثال :*/setstart* سلام چطوری؟ اگه پیامی داری واسه من همینجا بفرست😃—------------------------------🔹`4. `*/setdone *اگر شخصی در ربات برای شما پیامی بفرستد این پیام برای او ارسال میشود.مثال :*/setdone* پیامت رسید صبر کن تا جواب بدم➖➖➖➖➖➖➖➖➖➖➖🔶 لیست دکمه  های موجود :🔸`1.`*Send To All*ارسال پیام متنی به همه ی کاربران—------------------------------🔸`2.`*Members*تعداد کاربران—------------------------------🔸`3.`*Blocked Users*تعداد کاربران در لیست سیاه—-------------------------------🔸`4.`*Settings*تنظیمان ربات.","parse_mode" =>"MARKDOWN",'reply_markup' => array(
        'keyboard' => array(array('🗣 Send To All'),array('⚓️ Help','👥 Members','❌ Blocked Users'),array("Settings ⚙")),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true)));
        ...
        'resize_keyboard' => true)));
    		...
    		apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => 'سلام قربان حوش آمدید😉برای پاسخ روی پیام مورد نظر ریپلای کنید و متن خود را بنویسید 😎برای آشنایی دکمه ی ⚓️ Help️ را بزنید 👌😃.', 'reply_markup' => array(
        'keyboard' => array(array('🗣 Send To All'),array('⚓️ Help','👥 Members','❌ Blocked Users'),array("Settings ⚙")),
        'one_time_keyboard' => true,
        'selective' => true,
        'resize_keyboard' => true)));
	...
	if (!in_array($rpto,$banid)) {
    	...
    	apiRequest("sendMessage", array('chat_id' => $rpto, "text" => "<b>You Are Banned🚫,</b>-----------------شما در لیست سیاه قرار گرفتید🚫." ,"parse_mode" =>"HTML"));
    }
		apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "Banned➖➖➖➖➖➖➖➖➖➖➖به لیست سیاه افزوده شد." ,"parse_mode" =>"HTML"));
    		}
    	if($text == "/unban"){
